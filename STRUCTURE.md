...
- **`gui`**: Inventory-based user interface system. Uses `AuctifyHolder` for GUI state management.
...
- **`storage`**: Data persistence abstraction with SQLite and MySQL support using HikariCP for connection pooling.
- **`util`**: Utility classes for color handling, configuration, dependency downloading, and Discord webhooks.

## Data Models

- **`AuctionListing`**: A single representation of an auction. This object is thread-safe and handles internal bid validation.
...
- **Defensive Copying**: ItemStacks in listings are always cloned when stored or displayed to prevent accidental reference mutation.
